...
This project contains the files, software and instructions to build your own 3D CAD mouse. This is a device that allows for seamless 6-DOF navigation on any popular CAD software, however, the commercial options are quite pricy, so we took it in or own hands to make a (much cheaper, at about 20€) DIY option, that works just like the 150€ product. If you are a maker that spends a lot of time in CAD, or work with CAD professionally, consider making yourself one of these!

![Spacemouse](media/spacemouse.png)
...
A first prototype has been built, confirming that the PCB design is solid. All 6-DOF movements create their intended effect and allow for 3D navigation. No effort has been made yet to integrate the pushbuttons.

## Bill of Materials:

For each device that is built, the needed materials are:

 | Nº | Element | Quantity | Shopping link |
 | ---| ------- | -------- | ------------- |
 | 1  | Arduino Pro Micro | 1 | missing link |
 | 2  | Spacemouse PCB | 1 | missing link |
 | 3  | 3D printed parts | All | missing link |
 | 4  | 0.5 mm pitch, 5-pin ZIF connector | 4 | missing link |
 | 5  | Oculus Quest Joystick | 4 | missing link |
 | 6  | Wire | ~30 cm | missing link |
 | 7  | 6x6x4.3 mm pushbutton | 1 to 4 (user choice) | missing link |
 | 8  | 2-pin magnetic pogo connector| as many as pushbuttons | missing link |
 | 9  | USB cable (Arduino-Computer) | 1 | missing link |
...
- 3D printer (or the 3D printed parts)
- Soldering Iron
- Soldering flux
- Soldering Wick
- Tweezers (optional but *strongly* recommended)
- A computer that can flash an Arduino
...
3. Solder about 3cm wires to the pogo-pin connectors.
4. Solder about 4cm wires to the 6x6 mm pushbuttons.
...
11. Mount the knob assembly (...)
12. Carefully lift the PCB from the bottom part, and mount the knob assembly.

13. Place the 3D printed top part on top of the 3D printed bottom part.
...
1. Go to Arduino's [software page](https://www.arduino.cc/en/software/).
2. Select the IDE's version that matches your computer (eg: Windows, Linux).
3. Download and run the installation wizard.
